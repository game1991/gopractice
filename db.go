...
//联查查总数
func QueryCountNew(sql, sqlFind string, values ...interface{}) (int64, error) {
	sql = strings.Replace(sql, sqlFind, " count(*) as c ", -1)
	//查询数据条数
	totalMap, err := Query(sql, values...)
	if err != nil {
		return 0, err
	}

	total, err := strconv.ParseInt(totalMap[0]["c"].(string), 10, 64)
	if err != nil {
		return 0, err
	}
	return total, nil
